...
import {
  Wrench, AlertCircle, CalendarDays,
...
export default function ResourcePlan({ embedded = false, onSwitchToRap }: { embedded?: boolean; onSwitchToRap?: () => void }) {
  ...
  const costBasis = useMemo(() => readMarginSettings(project?.meta).costBasis, [project?.meta])
  const setCostBasis = useCallback((cb: 'rap' | 'rapp') => {
    if (!project?.id) return
    const next = { ...readMarginSettings(project.meta), costBasis: cb }
    updateProject(project.id, { meta: { ...project.meta, ...next } })
        ...
        <ModuleHeader
          ...
          <div className="flex gap-2">
            <div className="flex rounded-md border border-slate-200 dark:border-slate-700 overflow-hidden text-xs">
              <button
                type="button"
                onClick={() => setCostBasis('rap')}
                className={`px-2 py-1 ${costBasis === 'rap' ? 'bg-indigo-600 text-white' : 'text-slate-500 bg-white dark:bg-slate-900'}`}
              >
                Dasar: RAP
              </button>
              <button
                type="button"
                onClick={() => setCostBasis('rapp')}
                className={`px-2 py-1 ${costBasis === 'rapp' ? 'bg-indigo-600 text-white' : 'text-slate-500 bg-white dark:bg-slate-900'}`}
              >
                Dasar: RAPP
              </button>
            </div>
            ...
            <ExportMenu
              data={filtered}
              columns={[
                { header: 'Kode', accessor: r => r.resourceCode },
                { header: 'Nama', accessor: r => r.resourceName },
                { header: 'Tipe', accessor: r => TYPE_LABEL[r.resourceType] },
                { header: 'Satuan', accessor: r => r.unit },
                { header: 'Harga Satuan (AHSP)', accessor: r => r.unitPrice },
                { header: 'Volume Total', accessor: r => r.totalVolume },
                { header: 'Total Biaya', accessor: r => r.totalCost },
              ]}
              filename={`ResourcePlan_${projectId}`}
              size="sm"
              disabled={filtered.length === 0}
            />
          </div>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent className="p-4">
            <div className="mt-4 px-2 pb-10 overflow-x-auto">
              <div className="flex items-end gap-1.5 h-48 border-b border-slate-200 dark:border-slate-800" style={{ minWidth: `${arrivalSchedule.length * 40}px` }}>
                {arrivalSchedule.map(m => {
                  ...
                  const periodRatio = m.total / maxMonthly
                  const heatClass = periodRatio >= 0.75
                  ...
                  return (
                    <div
                      key={m.month}
                      className={`flex-1 flex flex-col justify-end relative group cursor-pointer h-full hover:bg-slate-50 dark:hover:bg-slate-800/20 ${heatClass}`}
            ...
            <div className="flex items-center gap-4 mt-4 pt-3 border-t border-slate-100 dark:border-slate-700 flex-wrap">
              ...
              <div className="ml-auto flex items-center gap-3 text-xs text-slate-400 border-l border-slate-100 dark:border-slate-700 pl-4">
                <span className="font-semibold text-slate-500">Intensitas:</span>
                <span className="flex items-center gap-1"><span className="inline-block w-3 h-0.5 bg-rose-400 rounded" />Tinggi ≥75%</span>
                <span className="flex items-center gap-1"><span className="inline-block w-3 h-0.5 bg-amber-400 rounded" />Sedang ≥45%</span>
                <span className="flex items-center gap-1"><span className="inline-block w-3 h-0.5 bg-emerald-400 rounded" />Rendah</span>
              </div>
            </div>
          </CardContent>
        </Card>
                    ...
                    return (
                      <React.Fragment key={type}>
                        ...
                        {rows.map(r => {
                          ...
                          const costShare = stats.totalCost > 0 ? r.totalCost / stats.totalCost : 0
                          const heatBgClass = costShare >= 0.15
                          ...
                          return (
                            <React.Fragment key={r.resourceId}>
                              <TableRow
                                className={`border-b border-slate-100 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/40 transition-colors cursor-pointer ${heatBgClass}`}
