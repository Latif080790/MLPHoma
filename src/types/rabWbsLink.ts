...
export interface RabWbsLink {
  ...
  /**
   * Origin and review state of this link.
   * 'auto'     = generated by fuzzy match or Auto-Schedule (needs human review).
   * 'reviewed' = confirmed correct by user after auto-generation.
   * 'manual'   = created directly by user in the WBS allocation UI.
   * 'rejected' = auto-generated link that was dismissed/overridden by user.
   */
  mappingStatus?: 'auto' | 'reviewed' | 'manual' | 'rejected'
  /**
   * Confidence score (0–100) for auto-generated links.
   * 100 = exact match or manually created. Lower = fuzzy/inferred.
   * Used to surface low-confidence links for review in the WBS UI.
   */
...
export function rowToLink(row: RabWbsLinkRow): RabWbsLink {
  return {
    ...
    mappingStatus: (row.mapping_status as RabWbsLink['mappingStatus']) ?? 'manual',
    mappingConfidence: row.mapping_confidence ?? 100,
