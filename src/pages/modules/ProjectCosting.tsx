...
function TabFallback() {
  return (
    <div className="flex flex-col gap-2 p-4">
      <div className="h-6 w-1/3 animate-pulse rounded" style={{ background: '#141d2e' }} />
      {[1, 2, 3, 4, 5].map(i => (
        <div key={i} className="h-8 animate-pulse rounded" style={{ width: `${90 - i * 8}%`, background: '#141d2e' }} />
      ))}
      <div className="mt-4 h-48 animate-pulse rounded opacity-60" style={{ background: '#141d2e' }} />
    </div>
  )
}
    ...
    <div
      ...
      style={isDark ? {
        ...
        backgroundImage:
          ...
          'linear-gradient(rgba(59,130,246,.01) 1px,transparent 1px),' +
          'linear-gradient(90deg,rgba(59,130,246,.01) 1px,transparent 1px)',
        backgroundSize: '80px 80px,80px 80px,16px 16px,16px 16px',
      } : { background: '#f4f6fb' }}
    >
      <h1 className="sr-only">Project Costing</h1>
      ...
      <div
        ...
        style={{
          ...
          boxShadow: 'inset 0 -1px 0 rgba(59,130,246,0.2), 0 1px 0 rgba(0,0,0,0.4)',
        }}
      >
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0 relative">
          ...
          {activeProject.code && (
            <span
              ...
              style={{ color: '#60A5FA', borderColor: 'rgba(59,130,246,0.7)' }}
            >
              {activeProject.code}
            </span>
          )}
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    height: 2,
                    borderRadius: 1,
                    background: done ? '#3B82F6' : 'rgba(255,255,255,0.07)',
                    boxShadow: done ? '0 0 6px rgba(59,130,246,0.6)' : 'none',
                  }}
                />
              )
            })}
      ...
      <div
        ...
        style={{ background: isDark ? '#0e1523' : '#f4f6fb', borderBottom: `1px solid ${isDark ? 'rgba(30,37,60,0.9)' : '#e2e6ef'}` }}
      >
        {/* Dashboard */}
        <button
          type="button"
          ...
          className={`flex items-center gap-1.5 px-4 h-full text-xs font-semibold flex-shrink-0 transition-all relative ${
            activeMode === 'dashboard'
              ? isDark ? 'text-[#60A5FA] bg-[#141d2e]' : 'text-blue-600 bg-white'
              : isDark ? 'text-[#7a90ab] hover:text-[#e2eaf5] hover:bg-[#141d2e]/60' : 'text-slate-500 hover:text-slate-700 hover:bg-white/60'
          }`}
          style={activeMode === 'dashboard' ? { boxShadow: 'inset 0 -2px 0 #3B82F6' } : {}}
        >
          <BarChart2 size={12} />
          Dashboard
        </button>
        ...
        <div className="flex items-center gap-0 flex-shrink-0">
          <div className="w-px h-5 mx-2" style={{ background: isDark ? 'rgba(255,255,255,0.06)' : '#dde1ea' }} />
          <span className="text-xs font-bold uppercase tracking-[0.1em] pr-2 leading-none" style={{ color: isDark ? '#3d5068' : '#94a3b8' }}>Pipeline</span>
        </div>
        ...
        {STEP_CONFIG.map((step, i) => {
          ...
          return (
            <React.Fragment key={step.id}>
              {i > 0 && (
                <div className="flex items-center flex-shrink-0" style={{ color: isDark ? '#3d5068' : '#c8cedc', padding: '0 2px' }}>
                  <ArrowRight size={10} />
                </div>
              )}
              <button
                ...
                className={`flex items-center gap-1.5 px-3 h-full text-xs font-medium flex-shrink-0 transition-all relative ${
                  isActive
                    ? isDark ? 'text-[#e2eaf5] bg-[#141d2e]' : 'text-slate-900 bg-white'
                    : isDark ? 'text-[#7a90ab] hover:text-[#e2eaf5] hover:bg-[#141d2e]/60' : 'text-slate-500 hover:text-slate-700 hover:bg-white/60'
                }`}
                style={isActive ? { boxShadow: 'inset 0 -2px 0 #3B82F6' } : {}}
              >
                <span style={{ color: isActive ? '#3B82F6' : isDark ? '#3d5068' : '#94a3b8' }}>{step.icon}</span>
                {step.label}
                {step.id !== 'resource' && (
                  <span
      ...
      {activeMode === 'pipeline' && (
        <div
          ...
          style={{ borderBottom: `1px solid ${isDark ? 'rgba(30,37,60,0.9)' : '#e2e6ef'}`, background: isDark ? '#131c2e' : '#f8f9fd' }}
        >
          {budgetCells.map((cell, i) => (
            <div
              key={cell.label}
              className="flex flex-col justify-center px-3 py-2"
              style={i > 0 ? { borderLeft: `1px solid ${isDark ? 'rgba(30,37,60,0.9)' : '#e2e6ef'}` } : {}}
            >
              <div
                className="text-xs font-bold uppercase leading-none mb-0.5"
                style={{ color: isDark ? '#3d5068' : '#9aa3b5', letterSpacing: '0.08em' }}
              >
                {cell.label}
              </div>
              <div
                className="text-sm font-bold font-mono leading-tight"
                style={{ color: cell.warn
                  ? (isDark ? '#F43F5E' : '#dc2626')
                  : i === 0
                    ? (isDark ? '#F59E0B' : '#d97706')
                    : (isDark ? 'rgba(226,234,245,0.9)' : '#1e293b') }}
              >
                {cell.value}
              </div>
              <div className="text-xs truncate leading-none mt-0.5" style={{ color: isDark ? '#3d5068' : '#9aa3b5' }}>
                {cell.sub}
              </div>
            </div>
          ))}
      ...
      {/* ── L4: Content ─────────────────────────────────────────────────────── */}
      <div className="flex-1 overflow-auto" style={{ background: isDark ? '#060910' : '#f4f6fb' }}>
        {activeMode === 'dashboard' ? (
          <CostDashboardView />
        ) : (
          <>
            {emptySteps.length === 4 && (
              <div className={`m-4 rounded-xl border border-dashed p-5 ${
                isDark ? 'border-amber-700/40 bg-amber-950/20' : 'border-amber-300 bg-amber-50/60'
              }`}>
                <div className="flex items-start gap-3">
                  <AlertCircle size={16} className={isDark ? 'text-amber-400 shrink-0 mt-0.5' : 'text-amber-600 shrink-0 mt-0.5'} />
                  <div className="space-y-2">
                    <p className={`text-sm font-semibold ${isDark ? 'text-amber-300' : 'text-amber-800'}`}>Pipeline Costing Belum Diisi</p>
                        ...
                        <React.Fragment key={step.id}>
                          <button
                            ...
                            className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors border ${
                              isDark
                                ? 'bg-[#131c2e] border-amber-700/30 text-amber-300 hover:bg-[#1a253c]'
