  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex justify-end gap-0.5 opacity-80 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
        <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-blue-600" onClick={(e) => { e.stopPropagation(); onHistoryClick(row.original) }}><History className="h-3.5 w-3.5" /></Button>
        <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-slate-900" onClick={(e) => { e.stopPropagation(); onEditItem(row.original) }}><Edit2 className="h-3.5 w-3.5" /></Button>
        <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-red-500 hover:bg-red-50" onClick={(e) => { e.stopPropagation(); onDeleteItem(row.original) }}><Trash2 className="h-3.5 w-3.5" /></Button>
      </div>
    ),
    size: 90,
  },
