...
import { runForwardPass } from '../../lib/autoScheduler'
...
export default function Timeline() {
  ...
  const { getTasks, removeTask, fetchTasks, updateTask } = useTimelineStore()
  ...
  /** Forward-pass CPM auto-schedule */
  const handleAutoSchedule = useCallback(() => {
    const allTasks = getTasks(projectId)
    if (!allTasks.length) return
    const projectStart = activeProject?.startDate ?? allTasks.reduce(
      (min, t) => (t.startDate < min ? t.startDate : min),
      allTasks[0].startDate
    )
    const updated = runForwardPass(allTasks, projectStart)
    updated.forEach((t) => {
      const orig = allTasks.find((o) => o.id === t.id)
      if (orig && (orig.startDate !== t.startDate || orig.endDate !== t.endDate)) {
        updateTask(projectId, t.id, { startDate: t.startDate, endDate: t.endDate })
      }
    })
    toast.success(`Auto-scheduled ${updated.length} tasks`)
  }, [getTasks, projectId, activeProject, updateTask])
